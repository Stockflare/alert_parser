module AlertParser
  describe Parser::VolumeTenDay do

    let(:locale) { Locale.new(language: 'en') }

    let(:currency_code) { 'usd' }

    let(:data) { { short_name: Faker::Company.name, price_change: rand(Math::E..Math::PI), timestamp: Time.now.utc.to_i } }

    let(:properties) { { locale: locale, currency_code: currency_code, data: data } }

    subject(:parser) { Parser::VolumeTenDay.new(properties) }
